...
st.title("🚀LLM para analise de Produção, by Waldemar Kubaski, Msc")
...
def get_top_team(start_date=None, end_date=None):
    filtered = df.copy()
    if start_date:
        filtered = filtered[filtered['Data'] >= pd.to_datetime(start_date)]
    if end_date:
    ...
    top = team_totals.sort_values('Total', ascending=False).iloc[0]
    return {
        "time": int(top.name),
        "total": int(top['Total']),
        "mp35": int(top['Terminal MP35']),
        "mp30": int(top['Terminal MP30'])
    }

def get_top_person(start_date=None, end_date=None):
    filtered = df.copy()
    if start_date:
        filtered = filtered[filtered['Data'] >= pd.to_datetime(start_date)]
    if end_date:
    ...
    top_name = person_totals.index[0]
    return {
        "nome": top_name,
...
def get_daily_production(start_date=None, end_date=None):
    ...
    return daily.reset_index().to_dict('records')[:10]
...
tools = [
    ...
    },
    {
        ...
        "function": {
            "name": "get_top_team",
            "description": "Retorna qual time produziu mais no período.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "get_top_person",
            "description": "Retorna o colaborador que mais produziu no período.",
            "parameters": {
                "type": "object",
                "properties": {
                    "start_date": {"type": "string"},
...
if prompt := st.chat_input("Ex: Qual time produziu mais em junho? Quem foi o melhor colaborador? Liste os colaboradores do Time 3."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Wald está analisando a planilha..."):
            try:
